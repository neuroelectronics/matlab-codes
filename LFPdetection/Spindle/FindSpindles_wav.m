function [ripples] = FindSpindles_wav(filtered,varargin)
...
restrict = [];
sd = [];
lowThresholdFactor = 580+50; % Ripple envoloppe must exceed lowThresholdFactor*stdev
highThresholdFactor = 650+50; % Ripple peak must exceed highThresholdFactor*stdev
minInterRippleInterval = 400; % in ms
maxRippleDuration = 2000; % in ms
minRippleDuration = 300; % in ms
noise = [];
pow_thresh = 150;
...
% Exclude ripple if it bridges a movement interval
fourthPass = ripples;
...
ripples_only_index = find(ripples(:, 1) ~= 0);
ripples = ripples(ripples_only_index, :);

% Exclude ripple if it has too low power
low_index = find(ripples(:, 4)<pow_thresh);
ripples(low_index, :) = [];
